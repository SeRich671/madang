@extends('layouts.admin', ['menuName' => 'Aktualizacja danych do faktury dla użytkownika ' . $user->first_name . ' ' . $user->last_name])

@section('content')
    <form method="POST" action="{{ route('admin.user.billing.update', ['user' => $user, 'billing' => $billing]) }}">
        @csrf
        @method('PUT')
