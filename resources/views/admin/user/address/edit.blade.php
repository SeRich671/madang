@extends('layouts.admin', ['menuName' => 'Aktualizacja adresu dla użytkownika ' . $user->first_name . ' ' . $user->last_name])

@section('content')
    <form method="POST" action="{{ route('admin.user.address.update', ['user' => $user, 'address' => $address]) }}">
        @csrf
        @method('PUT')
